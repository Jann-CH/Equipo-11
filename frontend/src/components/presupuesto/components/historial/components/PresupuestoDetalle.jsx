            ...
            <div className="bg-white border border-gray-100 shadow-sm p-4 rounded-[20px] flex flex-col gap-4">
                <div className="flex justify-between items-start">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="font-bold text-[#0B376D] text-lg">
                                {presupuesto?.cliente_nombre ?? "Cliente"} {presupuesto?.cliente_apellido ?? ""}
                            </h2>
                    ...
                    <span
                        className={`text-[11px] px-2.5 py-1 rounded-full font-semibold flex items-center gap-1 ${
                            presupuesto.estado === "Aceptado" || presupuesto.estado === "Aprobado"
                    ...
                    >
                        ...
                        {presupuesto.estado === "Guardado" ? "Pendiente" : presupuesto.estado}
                    </span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="bg-gray-50 border border-gray-100 p-3 rounded-2xl flex flex-col items-center text-center">
                        <span className="text-xs text-gray-400 font-medium">Fecha de creación</span>
                        <span className="text-sm font-bold text-[#0B376D] mt-0.5">
                            {presupuesto.fecha ? new Date(presupuesto.fecha).toLocaleDateString("es-AR", { day: '2-digit', month: 'long', year: 'numeric' }) : "-"}
                        </span>
                    </div>
                    <div className="bg-gray-50 border border-gray-100 p-3 rounded-2xl flex flex-col items-center text-center">
                        <span className="text-xs text-gray-400 font-medium">Vence en</span>
                        <span className="text-sm font-bold text-[#0B376D] mt-0.5">
                            {presupuesto.fecha_vencimiento ? new Date(presupuesto.fecha_vencimiento).toLocaleDateString("es-AR", { day: '2-digit', month: 'long', year: 'numeric' }) : "-"}
                        </span>
                    </div>
                </div>
                ...
                <div className="bg-white border border-gray-100 shadow-sm p-4 rounded-2xl flex flex-col gap-1">
                    ...
                    <span className="text-2xl font-extrabold text-[#0B376D]">
                        $ {parseFloat(presupuesto.total || 0).toLocaleString("es-AR", { minimumFractionDigits: 2 })}
                    </span>
                </div>

                {/* Detalles / Servicio */}
                <div className="flex flex-col gap-2 pt-2">
                    <h3 className="text-center font-bold text-sm text-[#0B376D] tracking-wider uppercase">Detalles</h3>
                    <div className="text-sm text-gray-700 space-y-1">
                        <p><strong className="text-[#0B376D]">Servicio:</strong> {presupuesto.servicio || "Sin servicio especificado"}</p>
                        <p><strong className="text-[#0B376D]">Items:</strong> {presupuesto.items || "Sin items"}</p>
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-1 pt-2">
                    ...
                    <p className="text-sm text-gray-600 bg-gray-50 p-3 rounded-2xl border border-gray-100">
                        {presupuesto.observacion || "Sin observaciones."}
                    </p>
                </div>
            </div>
